...
class Blockchain(DAG):
    ...
    def _get_longest_chain_tip(self, global_ids):
        ...
        return max(sorted(global_ids), key=lambda gid: self._G.nodes[gid][Blockchain._CHAIN_LENGTH_KEY])

    def __contains__(self, global_id):
        return global_id in self._G

    def __getitem__(self, global_id):
        return self._G.nodes[global_id][self._BLOCK_DATA_KEY]
    ...
    def add(self, block):
        ...
        if parent is not None:
            chain_length += self._G.nodes[parent][Blockchain._CHAIN_LENGTH_KEY]

        # add the block to the DAG
        self._G.add_node(copy_gid)
        self._G.nodes[copy_gid][Blockchain._CHAIN_LENGTH_KEY] = chain_length
        self._G.nodes[copy_gid][Blockchain._BLOCK_DATA_KEY] = block
        if parent is not None:
    ...
    def _update_longest_chain_incrementally(self, global_id, parent):
        ...
        """
        chain_length = self._G.nodes[global_id][Blockchain._CHAIN_LENGTH_KEY]
        if (self._longest_chain_tip_gid is None) or \
            (chain_length > self._G.nodes[self._longest_chain_tip_gid][Blockchain._CHAIN_LENGTH_KEY]) or \
            (chain_length == self._G.nodes[self._longest_chain_tip_gid][Blockchain._CHAIN_LENGTH_KEY] and
    ...
    def _chain_generator(self, tip_gid):
        ...
        if gid is None:
            return set()
        counter = self._G.nodes[gid][Blockchain._CHAIN_LENGTH_KEY] - 1
    ...
    def get_depth(self, global_id):
        ...
        if global_id not in self._longest_chain:
            return 0
        return self._G.nodes[self._longest_chain_tip_gid][Blockchain._CHAIN_LENGTH_KEY] - \
            self._G.nodes[global_id][Blockchain._CHAIN_LENGTH_KEY]
    ...
    def is_a_before_b(self, a, b):
        ...
        if a_in and (not b_in):
            return True
        return self._G.nodes[a][Blockchain._CHAIN_LENGTH_KEY] <= self._G.nodes[b][Blockchain._CHAIN_LENGTH_KEY]
