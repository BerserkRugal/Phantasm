...
class GreedyPHANTOM(PHANTOM):
    ...
    def _get_extreme_blue(self, global_ids: Collection[Block.GlobalID], bluest: bool = False) -> Block.GlobalID:
        ...
        return func(sorted(global_ids), key=lambda gid: self._G.nodes[gid][self._BLUE_NUMBER_KEY])
    ...
    def _coloring_chain_generator(self, tip_global_id: Block.GlobalID) -> Iterator[int]:
        ...
        while cur_gid is not None:
            yield cur_gid
            cur_gid = self._G.nodes[cur_gid][self._COLORING_PARENT_KEY]
    ...
    def _coloring_rule_2(self, k_chain: KChain, global_id: Block.GlobalID) -> bool:
        ...
        for cur_chain_block_gid in self._coloring_chain_generator(global_id):
            if self._G.nodes[cur_chain_block_gid][self._HEIGHT_KEY] < k_chain.minimal_height:
                return False
    ...
    def _coloring_rule_3(self, k_chain: KChain, global_id: Block.GlobalID) -> bool:
        ...
        depth = 0
        for cur_chain_block_gid in self._coloring_chain_generator(global_id):
            if (self._G.nodes[cur_chain_block_gid][self._HEIGHT_KEY] < k_chain.minimal_height) or (depth > self._k):
                return False
            if cur_chain_block_gid in k_chain.global_ids:
                return True
            depth += len(self._G.nodes[cur_chain_block_gid][self._BLUE_DIFF_PAST_ORDER_KEY])
        return False
    ...
    def _get_k_chain(self, global_id: Block.GlobalID) -> KChain:
        ...
        for cur_chain_block_gid in self._coloring_chain_generator(global_id):
            ...
            minimal_height = self._G.nodes[cur_chain_block_gid][self._HEIGHT_KEY]
            blue_count += len(self._G.nodes[cur_chain_block_gid][self._BLUE_DIFF_PAST_ORDER_KEY])
    ...
    def _update_diff_coloring_of_block(self, global_id: Block.GlobalID):
        ...
        parent_antipast = self._get_antipast(self._G.nodes[global_id][self._COLORING_PARENT_KEY])
        ...
        self._G.nodes[global_id][self._BLUE_DIFF_PAST_ORDER_KEY] = blue_diff_past_order
        self._G.nodes[global_id][self._RED_DIFF_PAST_ORDER_KEY] = red_diff_past_order
        self._G.nodes[global_id][self._BLUE_NUMBER_KEY] += \
            len(self._G.nodes[global_id][self._BLUE_DIFF_PAST_ORDER_KEY])

    def _is_a_bluer_than_b(self, a: Block.GlobalID, b: Block.GlobalID) -> bool:
        """
        :return: True iff the global id a is "bluer" than b
        """
        a_blue_number = self._G.nodes[a][self._BLUE_NUMBER_KEY]
        b_blue_number = self._G.nodes[b][self._BLUE_NUMBER_KEY]
    ...
    def _get_past(self, global_id: Block.GlobalID) -> AbstractSet[Block.GlobalID]:
        ...
        for cur_chain_gid, is_main_coloring_chain, is_intersection in \
            ...
            append_to.append(self._G.nodes[cur_chain_gid][self._BLUE_DIFF_PAST_ORDER_KEY])
            append_to.append(self._G.nodes[cur_chain_gid][self._RED_DIFF_PAST_ORDER_KEY])
    ...
    def _get_antipast(self, global_id: Block.GlobalID) -> AbstractSet[Block.GlobalID]:
        ...
        for cur_chain_gid, is_main_coloring_chain, is_intersection in \
            ...
            append_to.append(self._G.nodes[cur_chain_gid][self._BLUE_DIFF_PAST_ORDER_KEY].keys())
            append_to.append(self._G.nodes[cur_chain_gid][self._RED_DIFF_PAST_ORDER_KEY].keys())
    ...
    def _update_past_coloring_according_to(self, new_tip_gid: Block.GlobalID):
        ...
        for cur_chain_gid, is_main_coloring_chain, is_intersection in \
            ...
            if is_main_coloring_chain:
                ...
                blue_diff_past_orderings.append(self._G.nodes[cur_chain_gid][self._BLUE_DIFF_PAST_ORDER_KEY])
                red_diff_past_orderings.append(self._G.nodes[cur_chain_gid][self._RED_DIFF_PAST_ORDER_KEY])
    ...
    def _update_coloring_incrementally(self, global_id: Block.GlobalID):
        ...
        parents = self._G.nodes[global_id][self._BLOCK_DATA_KEY].get_parents()
        self._G.nodes[global_id][self._SELF_ORDER_INDEX_KEY] = None
        self._G.nodes[global_id][self._COLORING_PARENT_KEY] = \
            self._get_bluest(parents)
        self._G.nodes[global_id][self._BLUE_DIFF_PAST_ORDER_KEY] = set()
        self._G.nodes[global_id][self._RED_DIFF_PAST_ORDER_KEY] = set()

        if self._G.nodes[global_id][self._COLORING_PARENT_KEY] is not None:
            self._G.nodes[global_id][self._HEIGHT_KEY] = \
                max(self._G.nodes[parent][self._HEIGHT_KEY] for parent in parents) + 1
            self._G.nodes[global_id][self._BLUE_NUMBER_KEY] = \
                self._G.nodes[self._G.nodes[global_id][self._COLORING_PARENT_KEY]][self._BLUE_NUMBER_KEY]
        else:
            self._G.nodes[global_id][self._HEIGHT_KEY] = 0
            self._G.nodes[global_id][self._BLUE_NUMBER_KEY] = 0
    ...
    def _calculate_topological_order(self, coloring_parent_gid: Block.GlobalID, leaves: AbstractSet[Block.GlobalID],
        ...
        while to_order:
            ...
            if cur_parents <= ordered:
                ...
                to_order.extend(sort_blocks(self._G.nodes[cur_gid][self._COLORING_PARENT_KEY], coloring, cur_parents,
                                            unordered))
    ...
    def _update_topological_order_in_dicts(self, blue_dict: OrderDict, red_dict: OrderDict,
        ...
        if (coloring_parent_gid is not None) and \
                (self._G.nodes[coloring_parent_gid][self._SELF_ORDER_INDEX_KEY] is not None):
            starting_index = self._G.nodes[coloring_parent_gid][self._SELF_ORDER_INDEX_KEY]
        else:
            starting_index = 0
    ...
    def _update_self_order_index(self, global_id: Block.GlobalID):
        ...
        self._G.nodes[global_id][self._SELF_ORDER_INDEX_KEY] = \
            len(self._G.nodes[global_id][self._BLUE_DIFF_PAST_ORDER_KEY]) + \
            len(self._G.nodes[global_id][self._RED_DIFF_PAST_ORDER_KEY])
        coloring_parent_gid = self._G.nodes[global_id][self._COLORING_PARENT_KEY]
        if coloring_parent_gid is not None and \
                self._G.nodes[coloring_parent_gid][self._SELF_ORDER_INDEX_KEY] is not None:
            self._G.nodes[global_id][self._SELF_ORDER_INDEX_KEY] += \
                self._G.nodes[coloring_parent_gid][self._SELF_ORDER_INDEX_KEY]
    ...
    def _update_topological_order_incrementally(self, global_id: Block.GlobalID):
        ...
        self._update_topological_order_in_dicts(self._G.nodes[global_id][self._BLUE_DIFF_PAST_ORDER_KEY],
                                                self._G.nodes[global_id][self._RED_DIFF_PAST_ORDER_KEY],
    ...
    def get_depth(self, global_id: Block.GlobalID) -> float:
        ...
        depth = 1
        for cur_gid in self._coloring_chain_generator(self._coloring_tip_gid):
            if global_id in self._G.nodes[cur_gid][self._RED_DIFF_PAST_ORDER_KEY]:
                return 0
            if global_id in self._G.nodes[cur_gid][self._BLUE_DIFF_PAST_ORDER_KEY]:
                return depth
            depth += len(self._G.nodes[cur_gid][self._BLUE_DIFF_PAST_ORDER_KEY])
