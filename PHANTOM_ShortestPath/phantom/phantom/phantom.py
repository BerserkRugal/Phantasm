...
class PHANTOM(DAG):
    ...
    def get_virtual_block_parents(self) -> AbstractSet[Block.GlobalID]:
        ...
        sorted_leaves = sorted(self._leaves, key=lambda x:nx.shortest_path_length(self._G,x,self._genesis_gid), reverse=True)
        number_of_parents = np.random.poisson(1) + 1
        return set(sorted_leaves[:number_of_parents])
    ...
    def add(self, block: Block):
        ...
        self._G.nodes[global_id][self._BLOCK_DATA_KEY] = block
    ...
    def _update_coloring_incrementally(self, global_id: Block.GlobalID):
        ...
        for block, blue_anticone in max_coloring_bac.items():
            self._G.nodes[block][self._BAC_KEY] = blue_anticone
