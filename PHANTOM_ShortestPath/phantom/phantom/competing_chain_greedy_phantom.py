...
class CompetingChainGreedyPHANTOM(GreedyPHANTOM, MaliciousDAG):
    ...
    def add(self, block: Block, is_malicious: bool = False):
        ...
        if is_malicious:
            ...
            self._competing_chain_tip_antipast -= self._G.nodes[global_id][self._BLUE_DIFF_PAST_ORDER_KEY].keys()
            self._competing_chain_tip_antipast -= self._G.nodes[global_id][self._RED_DIFF_PAST_ORDER_KEY].keys()
    ...
    def _is_attack_viable(self) -> bool:
        ...
        return (self._G.nodes[self._coloring_tip_gid][self._BLUE_NUMBER_KEY] -
                self._G.nodes[self._competing_chain_tip_gid][self._BLUE_NUMBER_KEY]) <= self._maximal_depth_difference
    ...
    def get_virtual_block_parents(self, is_malicious: bool = False) -> AbstractSet[Block.GlobalID]:
        ...
        return set(list(self._virtual_competing_chain_block_parents)[:number_of_parents])

    def did_attack_succeed(self) -> bool:
        if self.did_attack_fail():
            return False

        # # Parasite Chain Attack Judgement
        ...
        #        (self.get_depth(self._currently_attacked_block_gid) == 0)

        # Splitting Attack Judgement
        return (self.get_depth(self._first_parallel_block_gid) >= self._confirmation_depth) and \
               (self._honest_dag.get_depth(self._currently_attacked_block_gid) >= self._confirmation_depth) and \
