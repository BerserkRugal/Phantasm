...
class TestGreedyColoring:
    ...
    def test_coloring_basic(self, greedy_dag, genesis, block1, block2, block3):
        ...
        assert greedy_dag._G.nodes[hash(genesis)][GreedyPHANTOM._HEIGHT_KEY] == 0
        assert greedy_dag._G.nodes[hash(genesis)][GreedyPHANTOM._BLUE_NUMBER_KEY] == 0
        assert greedy_dag._G.nodes[hash(genesis)][GreedyPHANTOM._BLUE_DIFF_PAST_ORDER_KEY].keys() == set()
        assert greedy_dag._G.nodes[hash(genesis)][GreedyPHANTOM._RED_DIFF_PAST_ORDER_KEY].keys() == set()
        assert greedy_dag._G.nodes[hash(genesis)][GreedyPHANTOM._COLORING_PARENT_KEY] is None
        ...
        assert greedy_dag._G.nodes[hash(block1)][GreedyPHANTOM._HEIGHT_KEY] == 1
        assert greedy_dag._G.nodes[hash(block1)][GreedyPHANTOM._BLUE_NUMBER_KEY] == 1
        assert greedy_dag._G.nodes[hash(block1)][GreedyPHANTOM._BLUE_DIFF_PAST_ORDER_KEY].keys() == {hash(genesis)}
        assert greedy_dag._G.nodes[hash(block1)][GreedyPHANTOM._RED_DIFF_PAST_ORDER_KEY].keys() == set()
        assert greedy_dag._G.nodes[hash(block1)][GreedyPHANTOM._COLORING_PARENT_KEY] == hash(genesis)
        ...
        assert greedy_dag._G.nodes[hash(block2)][GreedyPHANTOM._HEIGHT_KEY] == 1
        assert greedy_dag._G.nodes[hash(block2)][GreedyPHANTOM._BLUE_NUMBER_KEY] == 1
        assert greedy_dag._G.nodes[hash(block2)][GreedyPHANTOM._BLUE_DIFF_PAST_ORDER_KEY].keys() == {hash(genesis)}
        assert greedy_dag._G.nodes[hash(block2)][GreedyPHANTOM._RED_DIFF_PAST_ORDER_KEY].keys() == set()
        assert greedy_dag._G.nodes[hash(block2)][GreedyPHANTOM._COLORING_PARENT_KEY] == hash(genesis)
        assert greedy_dag.get_depth(hash(genesis)) == 1
        assert greedy_dag.get_depth(hash(block1)) == 0
        ...
        assert greedy_dag._G.nodes[hash(block3)][GreedyPHANTOM._HEIGHT_KEY] == 2
        assert greedy_dag._G.nodes[hash(block3)][GreedyPHANTOM._BLUE_NUMBER_KEY] == 3
        assert greedy_dag._G.nodes[hash(block3)][GreedyPHANTOM._BLUE_DIFF_PAST_ORDER_KEY].keys() == {hash(block1),
                                                                                                    hash(block2)}
        assert greedy_dag._G.nodes[hash(block3)][GreedyPHANTOM._RED_DIFF_PAST_ORDER_KEY].keys() == set()
        assert greedy_dag._G.nodes[hash(block3)][GreedyPHANTOM._COLORING_PARENT_KEY] == hash(block1)
                ...
                for leaf in chain(coloring_parent_list, blue_leaves, red_leaves):
                    ...
                    cur_leaf_order = \
                        self.get_topological_order(set(self._G.successors(leaf)),
                                                   self._G.nodes[leaf][GreedyPHANTOM._COLORING_PARENT_KEY])
