...
from typing import Iterable, Union, Set, AbstractSet
...
class Miner:
    ...
    def _fetch_block(self, block_gid):
        ...
        self._block_queue.nodes[block_gid][Miner._BLOCK_DATA_KEY] = None
    ...
    def _cascade_block_addition(self, block):
        ...
        while addition_queue:
            ...
            if cur_block_gid not in self._block_queue:
                continue
            cur_block = self._block_queue.nodes[cur_block_gid][Miner._BLOCK_DATA_KEY]
    ...
    def compute_score(self, candidates: AbstractSet[int]) -> (AbstractSet[int], float):

        number_of_parents = np.random.poisson(1) + 1
        sorted_cadidates = sorted(candidates, key=lambda x: self._dag[x].get_score(), reverse=True)
        temp_parents = sorted_cadidates[:number_of_parents]
        selected_parents = []

        cum_score = number_of_parents
        bsp = temp_parents[0]
        for sp in temp_parents:
            if self._dag[sp].get_score() > 0:
                tmp_score = self._dag[sp].get_score() * \
                             math.log(self.Alaph + self._dag[sp].get_score()/self._dag[bsp].get_score()) \
                             / math.log(self.Alaph + 1)
                if tmp_score > 0:
                    selected_parents.append(sp)
                    cum_score += tmp_score

        return set(selected_parents), cum_score
    ...
    def mine_block(self) -> Union[Block, None]:
        ...
        candidates = self._dag.get_virtual_block_parents()
