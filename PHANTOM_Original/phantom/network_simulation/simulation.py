...
class Simulation:
    ...
    def __init__(self,
        ...
        # self._env = simpy.RealtimeEnvironment(factor=0.1)
    ...
    def _block_generator_process(self) -> simpy.Event:
        ...
        while True:
            if len(self._network) > 0:
                ...
                # self.draw_dag("M6",with_labels=True)
                # if str(miner.get_name()) == "M6":
                #     self.draw_dag("M6")
            else:
    ...
    def end(self) -> bool:
        ...
        if not self._attack_success_event.triggered:
            ...
            self._log("attacked gid is: " + str(self._network["M6"]._dag._currently_attacked_block_gid) +
                      ", malicious depth is: " + str(self._network["M6"]._dag.get_depth(self._network["M6"]._dag._currently_attacked_block_gid)) +
                      ", honest dag depth is: " + str(self._network["M6"]._dag._honest_dag.get_depth(self._network["M6"]._dag._currently_attacked_block_gid)))
            self._log("parallel gid is: " + str(self._network["M6"]._dag._first_parallel_block_gid) +
                      ", malicious depth is: " + str(self._network["M6"]._dag.get_depth(self._network["M6"]._dag._first_parallel_block_gid)) +
                      ", honest dag depth is: " + str(self._network["M6"]._dag._honest_dag.get_depth(self._network["M6"]._dag._first_parallel_block_gid)))

        self._log("simulation ended")
        self._log(str(self._network))

        # self.draw_network(True)
        # self.draw_dag("H1",with_labels=True)
        # self.draw_dag("M6",with_labels=True)
