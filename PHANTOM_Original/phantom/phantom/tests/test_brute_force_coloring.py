...
class TestBruteForceColoring:
    ...
    def test_coloring_basic(self, brute_force_dag, genesis, block1, block2, block3):
        ...
        assert brute_force_dag._G.nodes[hash(genesis)][PHANTOM._BAC_KEY] == set()

        brute_force_dag.add(block1)
        assert brute_force_dag._get_coloring() == set(brute_force_dag._G.nodes())
        assert brute_force_dag._G.nodes[hash(genesis)][PHANTOM._BAC_KEY] == set()
        assert brute_force_dag._G.nodes[hash(block1)][PHANTOM._BAC_KEY] == set()

        brute_force_dag.add(block2)
        assert brute_force_dag._get_coloring() == set(brute_force_dag._G.nodes())
        assert brute_force_dag._G.nodes[hash(genesis)][PHANTOM._BAC_KEY] == set()
        assert brute_force_dag._G.nodes[hash(block1)][PHANTOM._BAC_KEY] == {hash(block2)}
        assert brute_force_dag._G.nodes[hash(block2)][PHANTOM._BAC_KEY] == {hash(block1)}
        ...
        assert brute_force_dag._G.nodes[hash(block1)][PHANTOM._BAC_KEY] == {hash(block2)}
        assert brute_force_dag._G.nodes[hash(block2)][PHANTOM._BAC_KEY] == {hash(block1)}
        assert brute_force_dag._G.nodes[hash(block3)][PHANTOM._BAC_KEY] == set()
